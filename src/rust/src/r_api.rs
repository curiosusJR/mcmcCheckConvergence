...
fn map_tree_err<T>(res: Result<T, String>) -> Result<T> {
    res.map_err(|e| Error::Other(e.into()))
}

#[extendr]
fn tree_clades(tree: String) -> Result<Vec<String>> {
    let (root, nodes) = map_tree_err(tree::parse_newick(&tree))?;
    map_tree_err(tree::collect_clades(root, &nodes))
}

#[extendr]
fn tree_tips(tree: String) -> Result<Vec<String>> {
    map_tree_err(tree::tree_tips(&tree))
}
...
fn clade_sets(trees: Vec<String>) -> Result<List> {
    ...
    for tree in trees.iter() {
        let (root, nodes) = map_tree_err(tree::parse_newick(tree))?;
        let clades = map_tree_err(tree::collect_clades(root, &nodes))?;
        out.push(clades);
    }
    let list_vals: Vec<Robj> = out.into_iter().map(|v| r!(v)).collect();
...
fn df_from_table(table: &core::Table) -> Result<Robj> {
    ...
    let list = if table.headers.is_empty() {
        ...
        List::from_names_and_values(&table.headers, &cols).map_err(|e| Error::Other(e.into()))?
    };
...
type RunsGensNames = (Vec<core::Run>, Vec<Option<f64>>, Vec<String>);

fn runs_from_list(list: &List) -> Result<RunsGensNames> {
...
fn runs_to_list(runs: &[core::Run], gens: &[Option<f64>], names: &[String]) -> Result<Robj> {
...
fn build_df_rows(rows: &[Vec<f64>], row_names: &[String], col_names: &[String]) -> Result<Robj> {
    ...
    for c in 0..ncol {
        ...
        for row in rows.iter().take(nrow) {
            col.push(*row.get(c).unwrap_or(&f64::NAN));
        }
        cols.push(r!(col));
    }
    let list = List::from_names_and_values(col_names, &cols).map_err(|e| Error::Other(e.into()))?;
    let mut df = Robj::from(list);
...
fn named_numeric(vec: &[(String, f64)]) -> Robj {
...
fn list_of_named(vecs: &[Vec<(String, f64)>], names: Option<&[String]>) -> Result<List> {
    let vals: Vec<Robj> = vecs.iter().map(|v| named_numeric(v)).collect();
...
fn ordered_union_names(vecs: &[Vec<(String, f64)>]) -> Vec<String> {
...
fn build_df_from_vecs(vecs: &[Vec<(String, f64)>], col_names: &[String]) -> Result<Robj> {
...
fn build_matrix_from_vecs(vecs: &[Vec<(String, f64)>], col_names: &[String]) -> Result<Robj> {
    if vecs.is_empty() || col_names.is_empty() {
        return Ok(r!(NULL));
    }
    let row_names = ordered_union_names(vecs);
...
fn table_splits(output: List, splits_per_run: bool) -> Result<Robj> {
    ...
    let ess = list_get(&tree_list, "ess").ok_or_else(|| Error::Other("Missing ess".into()))?;
    ...
    build_df_rows(
        &sorted_rows,
        &sorted_names,
        &["frequencies".to_string(), "ESS".to_string()],
    )
}
...
fn table_continuous(output: List) -> Result<Robj> {
    ...
    let means =
        list_get(&cont_list, "means").ok_or_else(|| Error::Other("Missing means".into()))?;
    let ess = list_get(&cont_list, "ess").ok_or_else(|| Error::Other("Missing ess".into()))?;
...
fn df_list(obj: &Robj, label: &str) -> Result<List> {
    obj.as_list()
        .ok_or_else(|| Error::Other(format!("Expected list or data.frame for {}", label)))
}
...
fn first_numeric(val: Robj) -> Result<f64> {
    ...
    if let Some(vals) = val.as_real_vector() {
        return Ok(vals.first().copied().unwrap_or(f64::NAN));
    }
    if let Some(vals) = val.as_integer_vector() {
        return Ok(vals.first().copied().unwrap_or(0) as f64);
    }
    Ok(f64::NAN)
}
...
fn plot_ks_data(output: List, precision: f64) -> Result<Robj> {
    ...
    let min_val = ks_values.iter().cloned().fold(minimum_ks, f64::min);
    let max_val = ks_values.iter().cloned().fold(minimum_ks, f64::max);
    ...
    let xlim = vec![
        min_val.min(minimum_ks) - 0.01,
        max_val.max(minimum_ks) + 0.01,
    ];
...
fn plot_ks_pooled_data(output: List, precision: f64) -> Result<Robj> {
    ...
    let minimum_ess = core::min_ess(precision);
    let minimum_ks = core::ks_threshold(0.01, minimum_ess);
    let max_val = all_vals.iter().cloned().fold(minimum_ks, f64::max);
    let breaks = seq_step(0.0, max_val.max(minimum_ks) + 0.01, 0.0023);
    let xlim = vec![
        0.0_f64.min(minimum_ks) - 0.01,
        max_val.max(minimum_ks) + 0.01,
    ];
    let ylim_top = rows.first().map(|r| r.len()).unwrap_or(0) as f64 - 2.0;
...
#[extendr]
fn plot_ess_splits_data(output: List, per_run: bool, precision: f64, breaks: Robj) -> Result<Robj> {
    let tree_params = list_get(&output, "tree_parameters")
        .ok_or_else(|| Error::Other("Missing tree_parameters".into()))?;
    let tree_list = tree_params
        .as_list()
    ...
    if per_run {
        ...
        if default_breaks.is_empty() {
            if let Some(first) = columns.first() {
                ...
                let max_val = vals.iter().cloned().fold(minimum_ess, f64::max);
        ...
        for (idx, col) in columns.iter().enumerate() {
            ...
            let run = list!(values = vals, name = name, x_top = x_top, y_top = y_top);
    ...
    let default_breaks = if breaks_vals.is_empty() {
        let max_val = all_vals.iter().cloned().fold(minimum_ess, f64::max);
...
fn read_trace(
    ...
    let mut table = tables.into_iter().next().unwrap_or(core::Table {
        headers: Vec::new(),
        columns: Vec::new(),
    });
    let nrows = table.nrows();
...
fn load_trees_internal(
    ...
    if log_path.is_none() && file.ends_with(tree_ext) {
...
fn clade_freq_tree(x: String) -> Result<Robj> {
    ...
    let df = List::from_names_and_values(["cladenames", "cladefreqs"], [r!(clades), r!(freqs)])
        .map_err(|e| Error::Other(e.into()))?;
    let mut obj = Robj::from(df);
    let _ = obj.set_attrib("class", r!(vec!["data.frame"]));
    Ok(obj)
...
fn check_clades_freq(runs: List, freq: f64) -> Result<List> {
    ...
    for run in parsed {
        ...
        if let Some(first) = run.trees.first() {
            let tips = tree_tips(first.clone())?;
            if !tips.is_empty() {
...
#[extendr]
fn ess_split_freq(runs: List, _tracer: bool) -> Result<List> {
    let (parsed, _gens, _names) = runs_from_list(&runs)?;
    let mut out = Vec::new();
    ...
    for (idx, run) in parsed.iter().enumerate() {
        ...
        if !vec_names.is_empty() {
            for clade in &vec_names {
                let Some(&id) = stats.index.get(clade) else {
                    continue;
                };
                ...
                let ess = core::ess_tracer(&is_split);
...
fn split_freq(runs: List, windows: bool) -> Result<Robj> {
    ...
    if !windows {
        ...
        for run in &parsed {
            ...
            for (id, freq) in freqs_by_id.iter_mut().enumerate() {
                *freq = if run.trees.is_empty() {
                    f64::NAN
                } else {
    ...
    for run in &parsed {
        ...
        for (id, freq) in freqs1.iter_mut().enumerate() {
            *freq = stats1.counts[id] as f64 / w1.len().max(1) as f64;
        }
        for (id, freq) in freqs2.iter_mut().enumerate() {
            *freq = stats2.counts[id] as f64 / w2.len().max(1) as f64;
        }
        let mut vec_splits = Vec::new();
        let mut vec_freqs = Vec::new();
        let mut vec_names = Vec::new();
...
fn check_convergence_r(
    ...
    let tree_compare = if result.tree_compare.is_empty() {
        list_of_named(&result.tree_compare, None).unwrap_or_else(|e| throw_r_error(e.to_string()))
    } else {
    ...
    let cont_params = List::from_names_and_values(
        ...
        [
            named_numeric(&result.cont_means),
            cont_ess_df.clone(),
            cont_compare_df.clone(),
        ],
    )
    .map_err(|e| Error::Other(e.into()))
    .unwrap_or_else(|e| throw_r_error(e.to_string()));
